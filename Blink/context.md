...
8. **New Support Cases feature (v18.83) — pending Snowflake credentials.** New screen + Dashboard tile pulling live case data from `PROD_MODELED.GTM.SUPPORT_CASES` via a new Worker route (`/snowflake/cases?owner=<name>&open=1`), filtered to the logged-in person's own currently-open cases (matched by their Google-profile name against `OWNER_NAME`). The Worker's `querySnowflake()`/`getSnowflakeJWT()` helpers (key-pair JWT auth, not a simple token) already existed in the Worker file before this route was added — they just weren't wired up to any HTTP endpoint yet. Still needs these Cloudflare secrets set before it'll actually work: `SNOWFLAKE_ACCOUNT`, `SNOWFLAKE_USER`, `SNOWFLAKE_PRIVATE_KEY`, `SNOWFLAKE_PUBLIC_KEY_FP` (SHA256 fingerprint of the public key, obtained via `DESCRIBE USER <username>` in Snowflake right after attaching the key), `SNOWFLAKE_WAREHOUSE`, `SNOWFLAKE_ROLE`. Until then, the screen/tile will show a connection error. Frontend pieces: `supportCasesState`, `fetchSupportCases()`, `renderSupportCases()` in app.js; nav item + `screen-supportcases`/`screen-supportcases-content` in index.html.
