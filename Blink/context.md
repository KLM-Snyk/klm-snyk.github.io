...
8. **New Support Cases feature (v18.83–84) — pending Snowflake OAuth Security Integration.** New screen + Dashboard tile pulling live case data from `PROD_MODELED.GTM.SUPPORT_CASES` via a new Worker route (`/snowflake/cases?owner=<name>&open=1&t=<token>`), filtered to the logged-in person's own currently-open cases (matched by their Google-profile name against `OWNER_NAME`). **Auth model (v18.84): per-user OAuth, not a shared service account** — each person signs into Snowflake themselves via a popup (`triggerSnowflakeOAuth()`, `/snowflake/start` → `/snowflake/callback`), exactly mirroring the existing Jira OAuth pattern, and their queries run under their own Snowflake identity/role. This replaced an earlier key-pair-JWT/shared-service-account design (`getSnowflakeJWT()`) that was removed in favor of this. Still needs a Snowflake admin to create an **OAuth Security Integration** (`OAUTH_CLIENT = CUSTOM`) with the Worker's `/snowflake/callback` as the registered redirect URI, which is what provides the `SNOWFLAKE_OAUTH_CLIENT_ID` / `SNOWFLAKE_OAUTH_CLIENT_SECRET` secrets — plus `SNOWFLAKE_ACCOUNT` and `SNOWFLAKE_WAREHOUSE`. Until then, both surfaces show a "Sign in to Snowflake" prompt (which will itself just error, since the integration doesn't exist yet). Frontend pieces: `supportCasesState`, `fetchSupportCases()`, `renderSupportCases()`, `triggerSnowflakeOAuth()` in app.js; nav item + `screen-supportcases`/`screen-supportcases-content` in index.html; a Settings section with sign-in/sign-out.
